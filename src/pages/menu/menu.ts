...
export class MenuPage {
  ...
  referencia;
  constructor(public tf : FileTransfer, public file : File, public fb : FirebaseApp, public media: Media, public storage: Storage, public navCtrl: NavController, public actionSheetCtrl: ActionSheetController) {
    ...
    storage.get('candidato' + i).then((candidato) => {
      if (candidato != null) {
        ...
        if (candidato != "marcado") {
          ...
          self.filetransfer = self.tf.create();
          try {
            var caminho = this.referencia.child('dir/' + candidato);

          }catch(err){
            semAudio = 1;
            console.log(candidato +"   sem audio");
            console.log(err);
          }
          if(semAudio = 0) {
            try {
            ...
            caminho.getDownloadURL().then(url => {
              console.log("url = " + url);
              self.filetransfer.download(url, self.file.externalDataDirectory + 'atual' + '.3gp').then(() => {
                console.log("caminho = " + self.file.externalDataDirectory);
                console.log("download efetuado");
                this.file.readAsDataURL(self.file.externalDataDirectory, 'atual' + '.3gp').then((x) => {
                  console.log("file = " + x);
                })
              })
            });
